...
func _ready() -> void:
	...
	for k in KINDS:
		...
		if rect.size == Vector2i.ZERO:
			out[k] = [1.0, 0.5]
			continue
		# how far above the feet the art reaches, and its half-width, both as a
		# fraction of the requested height
		var up := float(PAD - rect.position.y) / H
		var half := maxf(float(PAD - rect.position.x),
				float(rect.position.x + rect.size.x - PAD)) / H
		out[k] = [snappedf(up, 0.01), snappedf(half, 0.01)]
